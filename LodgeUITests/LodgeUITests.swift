...
class LodgeUITests: XCTestCase {
  let app = XCUIApplication()
  let pasteboard = NSPasteboard.general
  private let appBundleIdentifier = "com.nklmilojevic.Lodge"
  ...
  override func setUp() {
    ...
    let defaults = UserDefaults(suiteName: appBundleIdentifier)
    defaults?.set(true, forKey: "showInStatusBar")
    defaults?.set(true, forKey: "showSearch")
    defaults?.set("always", forKey: "searchVisibility")
  ...
  func testDisablesOnOptionClickingMenubarIcon() {
    clickStatusItem(withModifiers: .option)
  ...
  func testDisablesOnlyForNextCopyOnOptionShiftClickingMenubarIcon() {
    clickStatusItem(withModifiers: [.option, .shift])
  ...
  func testCloseWithRepeatedHotkeyWhileHoldingModifiers() throws {
    // Simulate the popup hotkey press (Cmd + Shift + C).
    let cDown = CGEvent(keyboardEventSource: nil, virtualKey: CGKeyCode(kVK_ANSI_C), keyDown: true)!
    cDown.flags = [.maskCommand, .maskShift]
    cDown.post(tap: .cghidEventTap)
    ...
    // Press C again while keeping the modifier keys pressed
    cDown.post(tap: .cghidEventTap)
    cUp.post(tap: .cghidEventTap)
  ...
  // Default interval for Lodge to check clipboard is 1 second
  ...
  private func clickStatusItem(withModifiers modifiers: XCUIElement.KeyModifierFlags = []) {
