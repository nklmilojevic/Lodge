...
class HistoryItem {
  ...
  func generateTitle() -> String {
    // Check content types instead of decoding the image. Touching `image` here
    // populated `cachedImage` for every image ever copied, and that cache is only
    // released explicitly - so a full-resolution bitmap stayed resident per item.
