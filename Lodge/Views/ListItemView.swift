  ...
  var body: some View {
    HStack(spacing: 0) {
      ...
      if let image {
        ...
        ListItemTitleView(attributedTitle: attributedTitle, fontSize: fontSize, title: title)
          .padding(.trailing, 5)
      }

      Spacer()

      // Shortcuts column removed by request.
    }
    ...
    .background(isSelected ? Color(nsColor: .selectedContentBackgroundColor) : .white.opacity(0.001))
    .popupCorners(minimumRadius: Popup.cornerRadius)
    .onHover { hovering in
      if hovering {
