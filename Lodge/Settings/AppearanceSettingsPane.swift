...
struct AppearanceSettingsPane: View {
  ...
  @Default(.ocrInImages) private var ocrInImages
  ...
  var body: some View {
    Settings.Container(contentWidth: 650) {
      ...
      Settings.Section(label: { Text("OCRInImages", tableName: "AppearanceSettings") }) {
        Toggle("", isOn: $ocrInImages)
          .labelsHidden()
          .help(Text("OCRInImagesTooltip", tableName: "AppearanceSettings"))
      }

      Settings.Section(label: { Text("FontSize", tableName: "AppearanceSettings") }) {
        HStack {
          TextField("", value: $fontSize, formatter: fontSizeFormatter)
            .frame(width: 120)
